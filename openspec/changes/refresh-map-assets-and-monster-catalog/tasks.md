...
- [x] 2.6 將森林、玩具城與台北的所有可見跳台統一改為 `image-native` 平台流程，移除同圖層中舊的自訂平面跳台混搭。
- [x] 2.7 針對圖片跳台補上近白像素淡出、左右邊緣融合與下緣淡化，避免白邊與突兀接縫。
...
- [x] 3.11 修正怪物顯示尺寸策略，保留來源圖長寬比；小型怪至少維持基準尺寸，大型怪則依原始圖尺寸保留較大外觀。
